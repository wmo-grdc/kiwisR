...
#' @examples
#' \dontrun{
#' ki_catchment_list(hub = 'swmc')
#' }
#'

ki_catchment_list <- function(hub, return_fields, datasource = 0) {


  # Account for user-provided return fields
  if (missing(return_fields)) {
    return_fields <- "catchment_no, catchment_id, catchment_name, catchment_parent_id, catchment_size, catchment_area_wkt, catchment_area_wkt_org"
  } else {
